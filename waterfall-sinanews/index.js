...
function getNode(news) {
  let html = `
              <li class="item">
                  ...
                  <p class="content">${news.short_intro}</p>
              </li>
          `;
  return $(html);
}
...
function waterFallPlace($node) {
  ...
  $("#pic-ct").height(Math.max.apply(null, itemArr)); //这里需要将父容器ul高度设置成数组中的最大高度,因为数组元素item都是绝对定位,父容器高度坍塌,需要设置高度,保证底部div#load不总是可见
}
...
function isVisible($el) {
  ...
  if (elTop < scrollTop + windowHeight + 500) {
    //还没有滚动至目标就开始加载数据
    return true;
  } else {
    return false;
